...
module onebitfulladder_tb;
    ...
    onebitfulladder adder(.S(S), .Cout(Cout), .A(A), .B(B), .Cin(Cin));
    ...
    initial begin
        $dumpfile("onebitfulladder.vcd"); //output file name
        $dumpvars(0, onebitfulladder_tb); //module to capture what level, 0 means all wire
    end
